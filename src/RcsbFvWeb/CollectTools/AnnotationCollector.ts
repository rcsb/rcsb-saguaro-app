            ...
            ann.features.forEach(d => {
                ...
                this.computeFeatureGaps(d.feature_positions).forEach(p => {
                    if(p.beg_seq_id != null) {
                        const key:string = p.end_seq_id != null ? p.beg_seq_id.toString()+":"+p.end_seq_id.toString() : p.beg_seq_id.toString() + this.rcsbAnnotationMap.getConfig(d.type).displayCoocurrence ? Math.random().toString(36).substr(2) : "";
