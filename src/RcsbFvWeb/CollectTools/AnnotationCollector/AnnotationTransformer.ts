...
export class AnnotationTransformer extends Map<string,RcsbFvTrackDataElementInterface> {
    ...
    public increaseElement(reference: SequenceReference, queryId: string, source: Source, targetId:string,d: Feature): void{
        computeFeatureGaps(d.feature_positions).forEach(e => {
            for(let i=e.beg_seq_id;i<=e.end_seq_id;i++){
                ...
                if( (this.get(key).value as number) > this.valueRange.max)
                    this.valueRange.max =  (this.get(key).value as number);
                if( (this.get(key).value as number) < this.valueRange.min)
                    this.valueRange.min =  (this.get(key).value as number);
            }
        });
    }
    ...
    private buildRcsbFvTrackDataElement(p: FeaturePositionGaps, d: Feature, targetId: string, source:Source, provenance:string): RcsbFvTrackDataElementInterface{
        ...
        const sourceId: string = source == Source.PdbInstance && this.entityInstanceTranslator != null ?
            targetId.split(Constants.instance)[0] + Constants.instance + this.entityInstanceTranslator.translateAsymToAuth(targetId.split(Constants.instance)[1]) : targetId;
        return {
            begin: p.beg_seq_id,
            end: p.end_seq_id,
