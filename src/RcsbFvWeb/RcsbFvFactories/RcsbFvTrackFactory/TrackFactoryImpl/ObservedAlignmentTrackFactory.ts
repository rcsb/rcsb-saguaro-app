...
export class ObservedAlignmentTrackFactory implements TrackFactoryInterface<[AlignmentRequestContextType, TargetAlignment]>{

    private alignmentTrackFactory: AlignmentTrackFactory;
    ...
    constructor(entityInstanceTranslator?: PolymerEntityInstanceTranslate) {
        this.alignmentTrackFactory = new AlignmentTrackFactory(entityInstanceTranslator);
    }
    ...
    public async getTrack(alignmentQueryContext: AlignmentRequestContextType, targetAlignment: TargetAlignment): Promise<RcsbFvRowConfigInterface> {
        return this.alignmentTrackFactory.getTrack(alignmentQueryContext, targetAlignment, this.alignedRegionToTrackElementList.bind(this));
    }
    ...
    private alignedRegionToTrackElementList(region: AlignedRegion, alignmentContext: AlignmentContextInterface): Array<RcsbFvTrackDataElementInterface>{
        ...
        return outRegions.map(r=>this.alignmentTrackFactory.addAuthorResIds({
            begin: r.query_begin,
            end: r.query_end,
